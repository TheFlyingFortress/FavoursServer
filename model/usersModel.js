...
function Users() {
        ...
        var db = require('monk')('localhost:5000/FavourServer');
        // obtain a reference to our cars collection within mongodb
        this.users = db.collection('usercollection');
};
 
// Retrieve a user by its id
Users.findById = function(id, success, error) {
        this.users.findById(id, response(success,error));
};
 
// Persist a new car document to mongodb
Users.create = function(user, success, error) {
        this.users.insert(user, response(success,error));
};
 
// Update an existing car document by id in mongodb
Users.update = function(user, success, error) {
        this.users.findAndModify(user._id,
                { $set: { username: user.username } }, response(success, error));
};
 
// Remove a car by id from the mongodb
Users.removeById = function(id, success, error) {
